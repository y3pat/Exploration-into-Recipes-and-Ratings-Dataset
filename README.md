...
My regression model uses a column transformer and a pipeline to change the 'submitted' and 'protein (PDV)' columns. The model first alters the submitted column so that it changes from an ordinal variable to a quantitative variable that it can work with by converting the values in the column to its year. It then converts the protein (PDV) column, which is a continuous variable, by using a quantiletransformer that replaces the protein value with whatever quantile it lies within, which is discrete since it can only take on a value between 0 and 100. Finally, it uses linear regression to predict the values in the rating column. For the purpose of this model, the dataframe will be changed such that na ratings do not show up. This is necessary since the model cannot perform if y has na values and since we don't know which values should be 0 and which should be na, it is safer to just remove them all rather than replacing with 0.
